...
static void copyRecord (IP2LocationRecord* cRec, IP2LocationRecord* pRec) {
	...
	cRec->elevation = pRec->elevation;
	cRec->usagetyp = pRec->usagetype;
}
...
static VALUE rb_IP2Location_get_elevation(VALUE class, VALUE ipaddr)
{
	IP2Location* IP2LocationObj = NULL;
	IP2LocationRecord* pRec = NULL;
	VALUE rRec;
	...
	VALUE ipObj = rb_iv_get(class, "@ip2loc");
	Data_Get_Struct(ipObj, IP2Location,  IP2LocationObj);
	pRec = IP2Location_get_elevation(IP2LocationObj, STR2CSTR(ipaddr));
	return rb_cIP2LocRec_new(cIP2LocRec, pRec);
}


static VALUE rb_IP2Location_get_usagetype(VALUE class, VALUE ipaddr)
{
	IP2Location* IP2LocationObj = NULL;
	IP2LocationRecord* pRec = NULL;
	VALUE rRec;
	...
	VALUE ipObj = rb_iv_get(class, "@ip2loc");
	Data_Get_Struct(ipObj, IP2Location,  IP2LocationObj);
	pRec = IP2Location_get_usagetype(IP2LocationObj, STR2CSTR(ipaddr));
	return rb_cIP2LocRec_new(cIP2LocRec, pRec);
}
...
static VALUE rb_cIP2LocRec_elevation (VALUE klass) {	
	IP2LocationRecord *pRec;
	Data_Get_Struct(klass, IP2LocationRecord, pRec);
	return rb_str_new2(pRec->elevation);
}

static VALUE rb_cIP2LocRec_usagetype (VALUE klass) {	
	IP2LocationRecord *pRec;
	Data_Get_Struct(klass, IP2LocationRecord, pRec);
	return rb_str_new2(pRec->usagetype);
}
...
void Init_IP2Location() {
  ...
  rb_define_method(cIP2Location, "get_elevation", rb_IP2Location_get_elevation, 1);
  rb_define_method(cIP2Location, "get_usagetype", rb_IP2Location_get_usagetype, 1);
  ...
  rb_define_method(cIP2LocRec, "timezone", rb_cIP2LocRec_timezone, 0);
	...
	rb_define_method(cIP2LocRec, "elevation", rb_cIP2LocRec_elevation, 0);
	rb_define_method(cIP2LocRec, "usagetype", rb_cIP2LocRec_usagetype, 0);
}                                         
